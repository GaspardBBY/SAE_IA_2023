...
public class TicTacToeState extends GameState {
    ...
    protected void updateGameValue() {
        if (lineThroughBoard()) { // un gagnant, lequel ?
            game_value = player_to_move == X ? Integer.MAX_VALUE : Integer.MIN_VALUE;
    ...
    public double evaluation(int exceptedPlayer) {
        ...
        for (int i = 0; i < 3; i++) {
            int rowStart = i * 3;
            score += evaluateLine(board[rowStart], board[rowStart + 1], board[rowStart + 2], exceptedPlayer);
        }

        for (int i = 0; i < 3; i++) {
            int colStart = i;
            score += evaluateLine(board[colStart], board[colStart + 3], board[colStart + 6], exceptedPlayer);
        }

        // Évaluation des diagonales
        score += evaluateLine(board[0], board[4], board[8], exceptedPlayer);
        score += evaluateLine(board[2], board[4], board[6], exceptedPlayer);

        return score;
    }

    private static int evaluateLine(char cell1, char cell2, char cell3, int exceptedPlayer) {
        int score = 0;

        if (cell1 == exceptedPlayer && cell2 == exceptedPlayer && cell3 == exceptedPlayer) {
            score = 100;
        } else if ((cell1 == exceptedPlayer && cell2 == exceptedPlayer) || (cell2 == exceptedPlayer && cell3 == exceptedPlayer)) {
            score = 50;
        }
