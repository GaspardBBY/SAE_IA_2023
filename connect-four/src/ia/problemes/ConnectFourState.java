...
public class ConnectFourState extends GameState {
    ...
    public void play(int col) {
        ...
        // Calculer la valeur du jeux -1,+inf,0.5,0
        game_value = -1;
    ...
    public double evaluation(int exceptedPlayer) {
        ...
        // Évaluation ligne
        score += evaluateDirection(0, 1, exceptedPlayer); // à droite
        score += evaluateDirection(0, -1, exceptedPlayer); // à gauche

        // Évaluation colonne
        score += evaluateDirection(1, 0, exceptedPlayer); // vers le bas

        // Évaluation diagonale (descendante)
        score += evaluateDirection(1, 1, exceptedPlayer); // bas droite
        score += evaluateDirection(1, -1, exceptedPlayer); // bas gauche

        // Évaluation diagonale (montante)
        score += evaluateDirection(-1, 1, exceptedPlayer); // haut droite
        score += evaluateDirection(-1, -1, exceptedPlayer); // haut gauche
    ...
    private double evaluateDirection(int dirRow, int dirCol, int exceptedPlayer) {
        ...
        for (int row = 0; row < board.length; row++) {
            for (int col = 0; col < board[0].length; col++) {
                score += evaluateLine(board, row, col, dirRow, dirCol, exceptedPlayer);
            }
        }
    ...
    private double evaluateLine(int[][] board, int row, int col, int dirRow, int dirCol, int exceptedPlayer) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (newRow >= 0 && newRow < board.length && newCol >= 0 && newCol < board[0].length) {
                if (board[newRow][newCol] == exceptedPlayer) {
                    count++;
                }
            }
        }
